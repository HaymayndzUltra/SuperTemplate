...
- **[STRICT]** Must respect human validation checkpoints (end of Phase 1, 3, 4, 5, 6) and stop until approval is recorded
...
- **`[MUST]`** Verify `scripts/script-registry.json` exists and is accessible
- **`[MUST]`** Validate all required scripts listed in Script Integration section are registered
- **`[MUST]`** Verify script dependencies (Python 3.10+, required libraries) are available
- **Evidence:** Script registry validation logged in `.artifacts/protocol-06-ai-use-case-definition/script-registry-validation.md`
- **Failure Handling:** If registry missing or incomplete, halt protocol execution, escalate to technical lead

### Stakeholder Response SLA
**[STRICT]** All stakeholder validation checkpoints have defined response timeframes:

- **Acknowledgment SLA:** Stakeholders must acknowledge request within 24 hours
- **Response SLA:** Stakeholders must provide approval/rejection within 72 hours
- **Escalation:** If no response within SLA, escalate to project manager
- **Conditional Approval:** If stakeholder unavailable, proceed with conditional approval and document assumptions
- **Evidence:** All SLA tracking documented in `.artifacts/protocol-06-ai-use-case-definition/stakeholder-response-log.md`
...
### Script Output Validation Procedures
**[STRICT]** After each automation script execution, validate outputs before proceeding:

1. **Schema Validation:**
   - Validate JSON outputs against defined schemas
   - Verify YAML/CSV/Markdown syntax compliance
   - Check required fields are present

2. **Data Quality Validation:**
   - Verify scores/values within expected ranges
...
5. **Failure Handling:**
   - Halt protocol if validation fails
   - Log validation errors with details
   - Require manual intervention before proceeding
   - Document in rollback-log.md
...
- Ethics and compliance approval (end of Phase 5)
- Final sign-off authorization (end of Phase 6)
...
1. **`[MUST]` Intake Raw AI Ideas:**
   ...
   * **Edge Cases:**
     ...
     - **No AI opportunities identified**: If no clear AI opportunities found:
       * Action: Document rationale in `no-ai-opportunities-analysis.md`
       * Escalation: Present to stakeholders with alternative recommendations
       * Options:
         a) Revisit business goals and discovery findings for missed opportunities
         b) Consider alternative approaches (non-AI automation, process improvement)
         c) Declare protocol incomplete if no viable AI path exists
       * Evidence: `.artifacts/protocol-06-ai-use-case-definition/phase-01-intake/no-ai-opportunities-analysis.md`
       * Handoff: If protocol terminated, create incomplete protocol report for Protocol 07
...
3. **`[MUST]` Validate Candidate Pool Completeness:**
   ...
   * **SLA:** If no response within 72 hours, escalate to project manager. If stakeholder unavailable, proceed with conditional approval, document assumptions
   * **Edge Cases:**
...
3. **`[MUST]` Validate Feasibility and Risk Assessments:**
   ...
   * **SLA:** If no response within 72 hours, escalate to project manager. If technical lead unavailable, proceed with conditional approval, document assumptions
   * **Edge Cases:**
...
2. **`[MUST]` Select Primary AI Use Cases:**
   ...
   * **Edge Cases:**
     - **No viable use cases after prioritization**: If no use cases meet selection criteria:
       * Action: Document root cause analysis in `viability-analysis.md`
       * Reassessment: Re-evaluate feasibility criteria, constraints, prioritization weights
       * Options:
         a) Adjust prioritization framework weights and recalculate
         b) Revisit Phase 2 specifications for missing context
         c) Escalate to stakeholders for scope adjustment
         d) If all alternatives exhausted: Declare protocol incomplete with full rationale
       * Evidence: `.artifacts/protocol-06-ai-use-case-definition/phase-04-prioritization/viability-analysis.md`, `.artifacts/protocol-06-ai-use-case-definition/reassessment-log.md`
       * Termination: If all alternatives exhausted, create `protocol-termination-report.md` for handoff
...
3. **`[MUST]` Validate Selection with Stakeholders:**
   ...
   * **SLA:** If no response within 72 hours, escalate to project manager. If stakeholders unavailable, proceed with conditional approval, document assumptions
   * **Edge Cases:**
...
### PHASE 5: MISUSE & ETHICS VALIDATION
...
3. **`[MUST]` Obtain Compliance Approval:**
   ...
   * **SLA:** If no response within 72 hours, escalate to project manager. If compliance officer unavailable, proceed with conditional approval, document assumptions
   * **Edge Cases:**
     ...
     - **All use cases fail ethics check**: If all selected use cases fail ethics/legal compliance:
       * Action: Document all rejections with detailed rationale in `ethics-failure-analysis.md`
       * Escalation: Present to steering committee with recommendations
       * Options:
         a) Adjust scope/constraints and re-run discovery (Phase 1)
         b) Consider alternative AI approaches or non-AI solutions
         c) Declare protocol incomplete, document rationale in `protocol-termination-report.md`
       * Evidence: `.artifacts/protocol-06-ai-use-case-definition/phase-05-ethics/ethics-failure-analysis.md`, `.artifacts/protocol-06-ai-use-case-definition/steering-committee-decision.md`
       * Handoff: If protocol terminated, create incomplete protocol report for Protocol 07 (no use cases)
...
1. **`[MUST]` Generate Final AI Use Case Specifications:**
   ...
   * **Note:** Artifact naming clarification:
     - `AI-project-workflow/protocols/06-ai-use-case-definition.md` = Protocol workflow document (this file)
     - `phase-05-signoff/ai-use-case-definition.md` = Final approved specifications artifact (output for Protocol 07)
     - These are different artifacts with different purposes and storage locations
   * **Edge Cases:**
...
3. **Rollback Steps**:
   - Phase 6 → Phase 5: Revert sign-off, restore draft status
   - Phase 5 → Phase 4: Revert compliance approval, restore selection state
...
### Gate 4: Misuse & Ethics Compliance (End of Phase 5)
...
### Gate 5: Final Sign-off Completion (End of Phase 6)
...
- [ ] **Quality Gates:** 5 validation checkpoints with clear pass/fail criteria
...
> "[MASTER RAY™ | PHASE 5 START] - Evaluating selected use cases for ethical and legal compliance."

> "[MASTER RAY™ | PHASE 6 START] - Generating final AI use case specifications and obtaining stakeholder sign-off."
...
| `no-ai-opportunities-analysis.md` | No AI opportunities analysis | .md | `phase-01-intake/` | Internal reference |
| `viability-analysis.md` | Viability analysis | .md | `phase-04-prioritization/` | Internal reference |
| `reassessment-log.md` | Reassessment log | .md | `root/` | Internal reference |
| `ethics-failure-analysis.md` | Ethics failure analysis | .md | `phase-05-ethics/` | Internal reference |
| `steering-committee-decision.md` | Steering committee decision | .md | `root/` | Internal reference |
| `protocol-termination-report.md` | Protocol termination report | .md | `root/` | Protocol 07 |
| `stakeholder-response-log.md` | Stakeholder response SLA tracking | .md | `root/` | Internal reference |
| `script-registry-validation.md` | Script registry validation | .md | `root/` | Internal reference |
