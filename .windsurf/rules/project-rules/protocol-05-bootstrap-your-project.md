...
| Gate 2: Repository Mapping | Repository structure captured, analysis plan approved by user, detected stack file generated | User approval recorded and stack detection coverage ≥ 90% | `repo-structure.txt`, `analysis-plan.md`, `detected-stack.json` | `validate_repo_mapping.py` |
...
| Gate 4: Governance Alignment | Documentation updates approved, rule audit final report passes, template inventory generated | Rule audit severity ≤ Medium and documentation approvals recorded | `documentation-plan.md`, `rule-audit-final.md`, `template-inventory.md` | `rules_audit_quick.py` |
...
```bash
# ❌ WRONG - Modifying production application files
# Editing src/main.tsx
# Deleting existing config files
# Modifying production environment variables

# ✅ CORRECT - Operating within permitted boundaries only
...
```markdown
<!-- ❌ WRONG - Proceeding without user validation -->
# Analysis Plan
- Proposed targets: package.json, src/main.tsx
- Status: PENDING
- Proceeding with analysis...

<!-- ✅ CORRECT - Waiting for user approval -->
# Analysis Plan
- Proposed targets: package.json, src/main.tsx
- Status: APPROVED
- Approved By: developer-id
...
```json
// ❌ WRONG - Low coverage, missing critical components
{
  "coverage": 65.0,
  "frontend": {
    "framework": "React"
...
// ✅ CORRECT - High coverage with complete stack details
{
  "coverage": 92.5,
  "frontend": {
    "framework": "React",
...
```
**Why:** Gate 2 requires stack detection coverage ≥ 90%. Low coverage indicates incomplete analysis, leading to missing architectural principles and incorrect context kit initialization. Downstream protocols depend on accurate stack detection.

**Anti-Pattern 4: Proceeding Without Validation Brief Approval**
```markdown
<!-- ❌ WRONG - Findings not validated by user -->
# Validation Brief
...
<!-- ✅ CORRECT - User validation incorporated -->
# Validation Brief
- Findings: 15 architectural principles identified
...
```markdown
<!-- ❌ WRONG - Rules not linked to documentation -->
# Project Rule: API Error Handling
- Rule: All API errors must use standardized envelope

<!-- ✅ CORRECT - Rules traceable to source documentation -->
# Project Rule: API Error Handling
- Rule: All API errors must use standardized envelope
...
```markdown
<!-- ❌ WRONG - Critical audit findings not resolved -->
# Rule Audit Report
- Status: FAIL
- Critical Issues: 3
...
<!-- ✅ CORRECT - All critical findings resolved -->
# Rule Audit Report
- Status: PASS
- Critical Issues: 0
...
```
**Why:** Gate 4 requires rule audit severity ≤ Medium and no critical findings. Critical audit findings indicate governance violations that break rule activation. Downstream protocols depend on valid governance rules.

**Anti-Pattern 7: Generating READMEs Without Approval**
```bash
# ❌ WRONG - READMEs generated without user approval
.artifacts/protocol-05/readme-updates/
  ├── README.md ✅ (generated, not approved)
  ├── ARCHITECTURE.md ✅ (generated, not approved)

# ✅ CORRECT - Each README approved before finalization
.artifacts/protocol-05/readme-updates/
  ├── README.md ✅ (approved by developer-id, 2025-01-30T13:00:00Z)
  ├── ARCHITECTURE.md ✅ (approved by developer-id, 2025-01-30T13:15:00Z)
  └── approval-log.md (contains all approval records)
```
...
```bash
# ❌ WRONG - No decision point documented
# Protocol 05 complete, proceeding to Protocol 02...

# ✅ CORRECT - Decision point documented with rationale
