...
AI/ML projects require standardized, validated protocols that ensure quality, compliance, and best practices throughout the complete machine learning lifecycle. Every protocol must pass 11 validation dimensions (≥ 0.95 score) to ensure consistency and maintainability.
...
```
AI-project-workflow/
├── 01-05: Foundation & Discovery (COPIED from .cursor/ai-driven-workflow/)
├── 06-09: AI Project Planning (NEW)
├── 10-14: Model Development (NEW)
├── 15-17: Model Testing & Quality (NEW)
├── 18-21: MLOps & Deployment (NEW)
├── 22-25: Monitoring & Maintenance (NEW)
├── 26-28: Governance & Closure (NEW)
└── 29-30: Optional Extensions (NEW)
```
...
**[STRICT]** All protocols MUST pass 11 validators with ≥ 0.95 score each:

### 5.1 Validator Suite
```bash
validators-system/scripts/
├── validate_protocol_identity.py       # Metadata, compliance
├── validate_protocol_role.py           # AI role definition
├── validate_protocol_workflow.py       # Steps, sequences
├── validate_protocol_gates.py          # Quality checkpoints
├── validate_protocol_scripts.py        # Automation hooks
├── validate_protocol_communication.py  # Status announcements
├── validate_protocol_evidence.py       # Artifact tracking
├── validate_protocol_handoff.py        # Integration points
├── validate_protocol_reasoning.py      # Decision logic
├── validate_protocol_reflection.py     # Continuous improvement
└── validate_all_protocols.py           # Master orchestrator
```
...
.artifacts/
├── protocol-[ID]/
│   ├── [artifact-name]-[timestamp].[ext]
│   ├── checksums.sha256
│   └── metadata.json
├── validation/
│   ├── protocol-[ID]-identity.json
│   ├── protocol-[ID]-role.json
│   ├── [... all 11 validator outputs]
│   └── protocol-[ID]-master-report.json
└── prd-progress.md
```
...
✅ Metadata includes: creator, timestamp, protocol_id, version
```
...
[QUALITY GATE PASSED: Gate 1] - Problem-AI Fit Score ≥ 0.8
...
## Phase Status
- Phase 0 (01-05): ✅ Complete
- Phase 1-2 (06-09): 🔄 In Progress
- Phase 3 (10-14): ⏳ Pending
- [...]
...
Model Quality: Consistent ≥ 0.90 F1 scores
