...
1. **`[STRICT]` Execute Baseline CI Validation:**
   * **1.1. Run CI Workflows:**
       ...
       * Communication: `"[MASTER RAY™ | PHASE 1 START] - Executing baseline CI workflows for quality audit enablement..."`
...
1. **`[STRICT]` Determine Review Mode and Protocol:**
   ...
   * **2.1. Resolve Review Mode:**
       ...
       * Communication: `"[MASTER RAY™ | PHASE 2 START] - Determining review mode '{mode}' via centralized router..."`
...
1. **`[STRICT]` Execute and Monitor Specialized Protocol:**
   * **3.1. Execute Selected Review Protocol:**
       ...
       * Communication: `"[MASTER RAY™ | PHASE 3 START] - Executing specialized review protocol '{protocol}'..."`
   ...
   * **3.3. Trigger Extended Checks for Comprehensive Mode:**
       * When mode == `comprehensive`, sequence quick → security → architecture → design → ui reviews
...
1. **`[STRICT]` Generate Audit Report Package:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 4 START] - Packaging unified quality audit deliverables..."`
   ...
   * Validation: Manifest completeness score ≥ 95%
...
|------|----------|----------------|----------|------------|
| Gate 1: Pre-Audit Automation | CI workflows succeed; coverage ≥ 80%; change-context artifact generated | Coverage ≥ 80%, zero CI blocking errors | `ci-*-results.json`, `coverage-report.json`, `change-context.json` | `run_protocol_4_pre_audit.py` |
...
| Gate 4: Unified Reporting | Audit package compiled; recommendation issued with signatures; artifacts checksum valid | Manifest completeness score ≥ 95%; approvals logged for decision | `quality-audit-manifest.json`, `readiness-recommendation.md`, package checksum file | `validate_gate_4_reporting.py` |
...
> Required action: Increase coverage to ≥80% or request waiver.
> 
> Options:
...
6. Manifest completeness score ≥ 95%
...
```bash
# ❌ WRONG - Specialized protocol executed without pre-audit automation
[MASTER RAY™ | PHASE 2 START] - Determining review mode...
# Missing: Phase 1 (CI workflows, coverage aggregation)
# Gate 1 cannot pass

# ✅ CORRECT - Pre-audit automation executed first
[MASTER RAY™ | PHASE 1 START] - Executing baseline CI workflows...
# CI workflows executed
# Coverage aggregated (85.5%)
# Change context generated
[MASTER RAY™ | PHASE 2 START] - Determining review mode...
```
**Why:** Gate 1 requires CI workflows succeed and coverage ≥ 80%. Bypassing pre-audit automation prevents baseline validation and causes incomplete audit results.

**Anti-Pattern 2: Coverage Below Threshold**
```json
// ❌ WRONG - Coverage below 80% threshold
{
...
// ✅ CORRECT - Coverage meets threshold
{
  "overall_coverage": 85.5,
...
```
**Why:** Gate 1 requires coverage ≥ 80%. Below-threshold coverage indicates insufficient test coverage and risks undetected defects.

**Anti-Pattern 3: Bypassing Specialized Protocol**
```bash
# ❌ WRONG - Report generated without executing specialized protocol
[MASTER RAY™ | PHASE 3 START] - Executing specialized review protocol...
# Protocol execution skipped
[MASTER RAY™ | PHASE 4 START] - Packaging unified quality audit deliverables...
# Report generated without protocol execution
# Gate 3 cannot pass

# ✅ CORRECT - Specialized protocol executed end-to-end
[MASTER RAY™ | PHASE 3 START] - Executing specialized review protocol 'comprehensive-review'...
...
```json
// ❌ WRONG - Blocker severity findings not resolved
{
  "findings_by_severity": {
...
// ✅ CORRECT - All blocker findings resolved or waived
{
  "findings_by_severity": {
    "CRITICAL": 0,
    "HIGH": 0,  // All resolved
...
```json
// ❌ WRONG - Router fallback not validated
{
  "router_decision": "comprehensive-review",
...
// ✅ CORRECT - Router fallback validated
{
  "router_decision": "comprehensive-review",
...
QUALITY-AUDIT-PACKAGE.zip:
  ├── ci-test-results.json ✅
  ├── ci-lint-results.json ✅
  ├── coverage-report.json ✅
  ├── audit-findings.json ✅
  ├── finding-summary.csv ✅
  ├── readiness-recommendation.md ✅
  └── quality-audit-manifest.json ✅
```
**Why:** Gate 4 requires manifest completeness score ≥ 95%. Incomplete packages prevent comprehensive review and cause downstream protocol failures.

**Anti-Pattern 7: Missing Readiness Recommendation**
```bash
# ❌ WRONG - Audit completed but recommendation not issued
.artifacts/quality-audit/
  ├── QUALITY-AUDIT-PACKAGE.zip ✅
  # Missing: readiness-recommendation.md
  # Gate 4 cannot pass

# ✅ CORRECT - Recommendation issued with decision
.artifacts/quality-audit/
  ├── QUALITY-AUDIT-PACKAGE.zip ✅
  └── readiness-recommendation.md ✅ (with decision: go/go-with-risks/no-go)
```
...
```json
// ❌ WRONG - Checksum verification failed
{
...
// ✅ CORRECT - Checksum verification passed
{
  "protocol_file": "comprehensive-review.md",
  "expected_checksum": "sha256:abc123...",
