...
**🚫 [CRITICAL] DO NOT deploy to production without recorded staging success, stakeholder approval, and an executable rollback plan.** Production deployment bridges pre-deployment validation and production operations. To ensure successful release, production deployments must validate readiness, execute controlled rollouts, monitor health windows, and maintain complete audit trails for recovery.
...
1. **`[STRICT]` Validate Pre-Deployment Evidence:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 1 START] - Verifying deployment readiness artifacts..."`
...
1. **`[STRICT]` Reconfirm Staging Health:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 2 START] - Reconfirming staging health prior to production launch..."`
...
2. **`[STRICT]` Execute Production Deployment:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 3 START] - Executing production deployment sequence..."`
...
3. **`[STRICT]` Run Immediate Post-Deployment Checks:**
   ...
   * Validation: Validation success rate ≥ 95%
...
1. **`[STRICT]` Monitor Health Window:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 4 START] - Monitoring post-deployment health window..."`
...
2. **`[STRICT]` Compile Release Report:**
   ...
   * Validation: Report completeness ≥ 95%
...
| Gate 3: Production Launch | Production approval recorded; deployment completed; immediate checks passed | 0 blocking incidents; validation success rate ≥ 95% | `production-approval.json`, `production-deployment-report.json`, `post-deployment-validation.json` | `validate_gate_11_launch.py` |
| Gate 4: Stabilization & Reporting | Health window metrics within thresholds; release report compiled; retrospective inputs documented | Metrics within SLO tolerances; report completeness ≥ 95% | `deployment-health-log.md`, `DEPLOYMENT-REPORT.md`, `retrospective-inputs.json` | `validate_gate_11_reporting.py` |
...
```json
// ❌ WRONG - Deployment started without approval
{
...
// ✅ CORRECT - Approval obtained before deployment
{
  "deployment_started": true,
  "production_approval": {
    "status": "approved",
...
```markdown
<!-- ❌ WRONG - Change freeze not acknowledged by all stakeholders -->
# Change Freeze Confirmation

- Team A: Acknowledged ✅
- Team B: Pending ⏳  <!-- Missing -->
- Team C: Acknowledged ✅

<!-- ✅ CORRECT - All stakeholders acknowledged -->
# Change Freeze Confirmation
...
```json
// ❌ WRONG - Validation failures not addressed
{
  "status": "fail",
...
```
**Why:** Gate 3 requires validation success rate ≥ 95%. Below-threshold validation indicates deployment issues that require rollback.

**Anti-Pattern 4: Metrics Outside SLO Tolerances**
```json
// ❌ WRONG - Metrics outside SLO tolerances
{
  "metrics": {
...
// ✅ CORRECT - Metrics within SLO tolerances
{
  "metrics": {
    "api_latency_p95": 250,
    "error_rate": 0.1,
...
```bash
# ❌ WRONG - Deployment complete but report not compiled
...
# ✅ CORRECT - Complete deployment report
.artifacts/deployment/
  ├── DEPLOYMENT-REPORT.md ✅ (completeness ≥ 95%)
  ├── DEPLOYMENT-EVIDENCE.zip ✅
  └── retrospective-inputs.json ✅
```
...
```json
// ❌ WRONG - Deployment ready but rollback not verified
{
  "readiness_checklist": {
    "rollback_verification": {
...
// ✅ CORRECT - Rollback plan verified
{
  "readiness_checklist": {
    "rollback_verification": {
      "status": "verified",
...
```json
// ❌ WRONG - Production deployment without staging reconfirmation
{
  "staging_validation": {
...
// ✅ CORRECT - Staging health reconfirmed before production
{
  "staging_validation": {
    "status": "confirmed",
...
```bash
# ❌ WRONG - Deployment complete but evidence incomplete
.artifacts/deployment/
  ├── production-deployment-report.json ✅
...
# ✅ CORRECT - Complete deployment evidence
.artifacts/deployment/
  ├── deployment-readiness-checklist.json ✅
  ...
  └── DEPLOYMENT-EVIDENCE.zip ✅
```
**Why:** Gate 4 requires report completeness ≥ 95%. Incomplete evidence prevents audit trail and recovery documentation.
