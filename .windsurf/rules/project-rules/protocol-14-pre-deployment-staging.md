...
1. **`[STRICT]` Confirm Upstream Approvals:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 1 START] - Validating upstream approvals and artifact completeness..."`
...
1. **`[STRICT]` Execute Deployment and Testing:**
   * **2.1. Run Staging Deployment Rehearsal:**
       ...
       * Communication: `"[MASTER RAY™ | PHASE 2 START] - Rehearsing deployment on staging environment..."`
   ...
   * **2.2. Validate Smoke & Acceptance Tests:**
       ...
       * Validation: Coverage ≥ 90% of targeted suites
...
1. **`[STRICT]` Validate Recovery and Compliance:**
   * **3.1. Rehearse Rollback Procedure:**
       ...
       * Communication: `"[MASTER RAY™ | PHASE 3 START] - Verifying rollback and recovery procedures..."`
...
1. **`[STRICT]` Assemble Go/No-Go Package:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 4 START] - Compiling pre-deployment readiness package for release approval..."`
...
|------|----------|----------------|----------|------------|
| Gate 1: Intake Confirmation | All upstream approvals verified; staging parity report free of critical drift | Completeness score = 100%; drift severity ≤ low | `intake-validation-report.json`, `staging-parity-report.json` | `validate_gate_10_intake.py` |
| Gate 2: Deployment Rehearsal | Deployment rehearsal successful; smoke/regression tests pass with acceptable coverage | 0 blocking errors; coverage ≥ 90% of targeted suites | `staging-deployment-run.log`, `staging-test-results.json` | `validate_gate_10_rehearsal.py` |
| Gate 3: Rollback & Security | Rollback rehearsal completes within RTO; security/compliance scans cleared | Recovery time ≤ RTO; zero unresolved blocking findings | `rollback-verification-report.json`, `security-compliance-report.json` | `validate_gate_10_security.py` |
| Gate 4: Readiness Approval | Go/no-go package complete; readiness approvals signed; deployment checklist updated | Manifest completeness ≥ 95%; approvals 100% recorded | `pre-deployment-manifest.json`, `readiness-approval.json`, `deployment-checklist.md` | `validate_gate_10_readiness.py` |
...
```json
// ❌ WRONG - Critical drift detected but not remediated
{
...
// ✅ CORRECT - Drift remediated or low severity
{
  "drift_severity": "none",
  "differences": [],
...
```
**Why:** Gate 1 requires drift severity ≤ low. Critical drift indicates staging doesn't mirror production, causing invalid rehearsal results.

**Anti-Pattern 2: Deployment Rehearsal Failures**
```json
// ❌ WRONG - Deployment rehearsal failed
{
  "status": "failure",
  "blocking_errors": 2,
  "errors": [
...
// ✅ CORRECT - Deployment rehearsal successful
{
  "status": "success",
  "blocking_errors": 0,
  "steps": [
...
// ✅ CORRECT - Test coverage meets threshold
{
  "scenarios_targeted": 20,
...
```
**Why:** Gate 2 requires coverage ≥ 90% of targeted suites. Below-threshold coverage indicates incomplete validation and risks missing deployment issues.

**Anti-Pattern 4: Rollback Exceeds RTO**
```json
// ❌ WRONG - Rollback exceeds RTO
{
  "rto_target_seconds": 600,
...
```
**Why:** Gate 3 requires recovery time ≤ RTO. Exceeding RTO indicates rollback procedures are insufficient and risks extended downtime.

**Anti-Pattern 5: Security Blocking Findings**
```json
// ❌ WRONG - Security scan has blocking findings
{
...
// ✅ CORRECT - No blocking security findings
{
...
# ✅ CORRECT - Complete pre-deployment package
PRE-DEPLOYMENT-PACKAGE.zip:
  ├── intake-validation-report.json ✅
  ├── staging-parity-report.json ✅
  ├── staging-deployment-run.log ✅
  ├── staging-test-results.json ✅
  ├── rollback-verification-report.json ✅
  ├── security-compliance-report.json ✅
  ├── readiness-approval.json ✅
  ├── deployment-checklist.md ✅
  └── pre-deployment-manifest.json ✅
```
**Why:** Gate 4 requires manifest completeness ≥ 95%. Incomplete packages prevent comprehensive production readiness review.

**Anti-Pattern 8: Deployment Checklist Not Updated**
```bash
# ❌ WRONG - Rehearsal complete but checklist not updated
...
# ✅ CORRECT - Checklist updated with rehearsal learnings
.artifacts/pre-deployment/
  ├── staging-deployment-run.log ✅
  ├── readiness-approval.json ✅
  └── deployment-checklist.md ✅ (updated with production steps)
```
