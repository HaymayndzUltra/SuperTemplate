...
1. **`[STRICT]` Monitor Active Alerts:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 1 START] - Monitoring production alerts for incident signals..."`
...
1. **`[STRICT]` Identify Mitigation Options:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 2 START] - Identifying mitigation strategy for incident containment..."`
...
1. **`[STRICT]` Execute Mitigation or Rollback:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 3 START] - Executing approved mitigation/rollback actions..."`
...
2. **`[STRICT]` Validate System Recovery:**
   ...
   * Validation: Recovery validation success rate ≥ 95% of critical checks
...
1. **`[STRICT]` Confirm Incident Resolution:**
   ...
   * Communication: `"[MASTER RAY™ | PHASE 4 START] - Confirming incident resolution and notifying stakeholders..."`
...
| Gate 3: Recovery Validation | Mitigation executed successfully; recovery validation passed; timeline updated | Recovery validation success rate ≥ 95% of critical checks | `mitigation-execution-report.json`, `recovery-validation.json`, `incident-timeline.md` | `validate_gate_13_recovery.py` |
| Gate 4: Resolution & Documentation | Resolution summary recorded; root cause evidence archived; incident report drafted | Documentation completeness ≥ 95%; required stakeholders informed | `resolution-summary.json`, `rca-manifest.json`, `INCIDENT-REPORT.md` | `validate_gate_13_resolution.py` |
...
3. `recovery-validation.json` shows recovery validation success rate ≥ 95%
...
```json
// ❌ WRONG - Rollback executed without severity consensus
{
  "incident_id": "INC-2025-001",
...
// ✅ CORRECT - Severity confirmed before rollback
{
  "incident_id": "INC-2025-001",
  "severity": "SEV-1",
  "consensus_reached": true,
...
```json
// ❌ WRONG - Rollback attempted without prerequisites
{
  "rollback_readiness": {
...
// ✅ CORRECT - All prerequisites verified
{
  "rollback_readiness": {
...
```json
// ❌ WRONG - Recovery validation below 95%
{
...
// ✅ CORRECT - Recovery validation meets threshold
{
  "validation_success_rate": 98,
  "critical_checks_total": 27,
  "critical_checks_passed": 26,
...
```
**Why:** Gate 3 requires recovery validation success rate ≥ 95% of critical checks. Below-threshold validation indicates incomplete recovery and risks recurring incidents.

**Anti-Pattern 4: Missing Mitigation Approval**
```json
// ❌ WRONG - Mitigation executed without approval
{
...
// ✅ CORRECT - Approval obtained before execution
{
  "mitigation_plan": "Rollback backend service",
  "decision_approvals": {
    "status": "approved",
...
```bash
# ❌ WRONG - Incident closed but evidence missing
.artifacts/incidents/
  ├── incident-intake-log.md ✅
  ├── severity-assessment.json ✅
  ├── mitigation-execution-report.json ✅
  # Missing: recovery-validation.json
  # Missing: rca-manifest.json
  # Documentation completeness < 95%

# ✅ CORRECT - Complete root cause evidence
.artifacts/incidents/
  ├── incident-intake-log.md ✅
  ├── severity-assessment.json ✅
  ├── mitigation-plan.md ✅
  ...
  └── INCIDENT-REPORT.md ✅
```
**Why:** Gate 4 requires documentation completeness ≥ 95%. Missing evidence prevents postmortem analysis and causes learning gaps.

**Anti-Pattern 6: Incident Closed Without Resolution Confirmation**
```json
// ❌ WRONG - Incident closed without resolution confirmation
{
  "incident_id": "INC-2025-001",
  "status": "closed",
  "resolution_summary": {
...
// ✅ CORRECT - Resolution confirmed before closure
{
  "incident_id": "INC-2025-001",
  "status": "closed",
  "resolution_summary": {
