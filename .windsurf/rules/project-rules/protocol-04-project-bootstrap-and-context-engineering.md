...
1. **`[STRICT]` Verify Required Artifacts:**
   ...
   * Verify `project-brief-validation-report.json` from Protocol 03 exists with status = "PASS", score ≥ 0.95
...
2. **`[STRICT]` Normalize Governance Rules:**
   ...
   * Validation: No critical issues in audit report (severity ≤ Medium)
...
|------|----------|----------------|----------|------------|
| Gate 1: Brief Validation | Project Brief validation report status = PASS and approvals present | Validation score ≥ 0.95 | `brief-validation-report.json` | `validate_brief.py` |
| Gate 2: Environment & Rule Integrity | Environment doctor passes and rule audit reports no critical issues | Doctor exit code 0, audit severity ≤ Medium | `environment-report.json`, `rule-audit-report.md` | `doctor.py`, `rules_audit_quick.py` |
| Gate 3: Scaffold Validation | Scaffold manifest matches registry, validation report status = PASS | Compliance ≥ 98% | `bootstrap-manifest.json`, `scaffold-validation-report.json` | `validate_scaffold.py` |
...
```bash
# ❌ WRONG - Modifying existing application code
...
# ✅ CORRECT - Creating backup before operations
...
```json
// ❌ WRONG - Compliance below threshold
{
...
// ✅ CORRECT - Compliance meets threshold
{
  "compliance_score": 98.5,
  "validation_status": "PASS",
  "issues": [],
...
```
**Why:** Gate 3 requires compliance ≥ 98%. Low compliance scores indicate incomplete or incorrect scaffold generation. Downstream protocols depend on complete scaffold structures for project initialization.

**Anti-Pattern 5: Skipping Dry Run**
```bash
# ❌ WRONG - Generating scaffold without preview
...
# ✅ CORRECT - Previewing operations before execution
...
# Verify no critical issues (severity ≤ Medium)
# Proceed with scaffold generation
```
...
```json
// ❌ WRONG - Missing required approvals
{
  "brief_validation": {
    "status": "PASS",
    "score": 0.97
...
// ✅ CORRECT - All approvals documented
{
  "brief_validation": {
    "status": "PASS",
    "score": 0.97
