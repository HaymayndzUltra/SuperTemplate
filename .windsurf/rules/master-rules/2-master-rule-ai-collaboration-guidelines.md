...
*   **[STRICT]** **Research Announcement:** You **MUST** announce: `[RAY MINDZ | PLATFORM RESEARCH] Consulting {Platform} documentation for {Feature} to ensure native implementation patterns.`
...
*   **[STRICT]** **Exception:** This protocol **MUST NOT** be used if the AI is already executing a task from a pre-existing technical plan file (e.g., `tasks-*.md`), as defined in protocols like `10-process-tasks.md`. In that scenario, the Markdown file is the sole source of truth for the task list.
*   **[STRICT]** **Phase 1: Planning:**
    1.  Based on the user's request and initial analysis, formulate a high-level plan.
    2.  Present this plan to the user for validation using the `[RAY MINDZ | PROPOSED PLAN]` format.
...
*   **[STRICT]** **Phase 3: Execution and Progress Updates:**
    ...
    3.  Announce task completion concisely: `[RAY MINDZ | TASK COMPLETED] {task_name}`.
...
*   **[STRICT]** **Scenario 1: Direct Conflict.** If a user instruction contradicts an existing rule:
    ...
    *   **Communication:** Notify the user using this strict format: `[RAY MINDZ | RULE CONFLICT] The instruction "{user_instruction}" conflicts with the rule "{rule_name}," which states: "{quote_from_rule}". How should I proceed?`
*   **[STRICT]** **Scenario 2: Uncertainty or Ambiguity.** If the user's request is ambiguous, incomplete, or if multiple approaches are possible:
    ...
    *   **Communication:** Ask a concise clarification question. `[RAY MINDZ | CLARIFICATION QUESTION] {your_question_here}`. Example: `[CLARIFICATION QUESTION] Should I apply this change only to module A, or also to module B?`
...
- **[STRICT]** All messages related to the application of these meta-rules **MUST** use the formalisms defined below:
    ...
    - `[RAY MINDZ | RULE IMPROVEMENT SUGGESTION]`
    - `[RAY MINDZ | GOVERNANCE MODIFICATION PROPOSAL]`
    - `[RAY MINDZ | CONTEXT REFRESH SUGGESTION]`
    - `[RAY MINDZ | RECOMMENDATION INITIAL]`
    - `[RAY MINDZ | AUTO-CRITIQUE OBJECTIVE]`
    - `[RAY MINDZ | RECOMMENDATION FINAL VALIDATED]`
