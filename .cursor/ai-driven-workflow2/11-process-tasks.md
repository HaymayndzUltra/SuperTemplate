...
Evidence: Track initialization in `.artifacts/protocol-10/workflow-logs/`
...
Evidence: Store artifacts in `.artifacts/protocol-10/`
...
- State stored in: `.artifacts/protocol-10/workflow-state.json`
- Checkpoint validation at each step boundary
- Rollback procedure if step fails: Return to previous step and remediate

### Workflow Monitoring

- Real-time status: `.artifacts/protocol-10/workflow-status.json`
- Execution logs: `.artifacts/protocol-10/workflow-logs/`
- Performance metrics: `.artifacts/protocol-10/workflow-metrics.json`
...
- `ARTIFACTS_DIR=.artifacts/protocol-10/` - Artifacts storage location
- `LOG_LEVEL=INFO` - Logging verbosity (DEBUG, INFO, WARNING, ERROR)

**Required Permissions:**
- Read access to: `.cursor/ai-driven-workflow/10-*.md`, `.artifacts/`
- Write access to: `.artifacts/protocol-10/`, `scripts/logs/`
...
**Output:** `.artifacts/protocol-10/prerequisites-validation.json`
...
python3 scripts/run_protocol_gates.py \
  --protocol 10 \
  --input .artifacts/protocol-10/input/ \
  --output .artifacts/protocol-10/output/ \
  --log-file .artifacts/protocol-10/execution.log \
...
python3 scripts/aggregate_evidence_10.py \
  --protocol 10 \
  --artifacts-dir .artifacts/protocol-10/ \
  --output-manifest \
  --checksum sha256
```
**Flags:**
...
**Output:** `.artifacts/protocol-10/EVIDENCE-MANIFEST.json`
...
**Output:** `.artifacts/protocol-10/validation-report.json`
**Exit Codes:** 0=all gates pass, 1=gate failure, 2=critical error

### Error Handling & Fallback Procedures

**If Command 1 (Prerequisites) Fails:**
1. Check log: `.artifacts/protocol-10/prerequisites-validation.json`
...
1. Check log: `.artifacts/protocol-10/execution.log`
...
**Log Files:**
- `.artifacts/protocol-10/execution.log` - Main execution log
- `.artifacts/protocol-10/validation.log` - Validation log
- `.artifacts/protocol-10/error.log` - Error log (if any)

**Status Files:**
- `.artifacts/protocol-10/workflow-status.json` - Real-time status
- `.artifacts/protocol-10/workflow-metrics.json` - Performance metrics
...
✅ All artifacts stored in `.artifacts/protocol-10/`
...
- [ ] All artifacts generated and stored in `.artifacts/protocol-10/`
...
- Log feedback in `.artifacts/protocol-10/feedback-log.json`
