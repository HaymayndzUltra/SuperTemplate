...
### Protocol Identity
- **Protocol ID:** 01
- **Protocol Name:** Client Proposal Generation
- **Protocol Owner:** Proposal Writer/Freelancer
...
### Protocol Lineage
- **Predecessor:** None (first protocol in workflow)
- **Successor:** Protocol 02 (Client Discovery Initiation)
- **Related Protocols:** Protocol 03 (Project Brief Creation)

### Protocol Metadata
- **Purpose:** Transform raw job posts into human-voice freelance proposals with evidence tracking
- **Success Criteria:** Proposal passes all 5 quality gates with human voice compliance ≥0.95
- **Failure Modes:** AI detection, forbidden phrases, pricing misalignment, missing artifacts
- **Recovery Procedure:** Return to relevant phase, remediate issues, re-run validation suite
...
#### Protocol Executor
- **Role:** Proposal Writer/Freelancer
- **Responsibilities:**
  - Execute protocol steps in sequence
  - Validate at each checkpoint
  - Escalate blockers immediately
  - Document decisions and rationale
- **Authority:** Can make tone and pricing decisions within guidelines
- **Escalation:** Technical Lead for validation failures

#### Protocol Owner
- **Role:** Proposal Writer/Freelancer (same as executor for Protocol 01)
- **Responsibilities:**
  - Approve protocol execution
  - Review quality gates
  - Sign off on handoff
  - Address escalations
- **Authority:** Final decision on proposal content and pricing

#### Downstream Owner
- **Role:** Discovery Call Lead (Protocol 02 owner)
- **Responsibilities:**
  - Receive handoff package
  - Validate inputs from this protocol
  - Provide feedback on quality
  - Confirm readiness for next phase
- **Authority:** Can request proposal revisions before discovery call

### Role Interactions
- **Executor → Owner:** Self-review for Protocol 01 (same person)
- **Owner → Downstream:** Handoff via evidence bundle and proposal-summary.json
- **Downstream → Executor:** Feedback on proposal quality and client response
...
| `validate_gate_01_compliance.py` | Validate Gate 01 Compliance | `scripts/` | ✅ Exists |
| `validate_gate_01_structure.py` | Validate Gate 01 Structure | `scripts/` | ✅ Exists |
| `aggregate_evidence_01.py` | Aggregate Evidence 01 | `scripts/` | ✅ Exists |
| `validate_gate_01_final.py` | Validate Gate 01 Final | `scripts/` | ✅ Exists |
| `run_protocol_gates.py` | Run Protocol Gates | `scripts/` | ✅ Exists |
| `validate_gate_01_jobpost.py` | Validate Gate 01 Jobpost | `scripts/` | ✅ Exists |
| `validate_gate_01_tone.py` | Validate Gate 01 Tone | `scripts/` | ✅ Exists |
...
----------------|---------|----------|--------|
| `analyze_jobpost.py` | Analyze Jobpost | `scripts/` | ✅ Exists |
| `tone_mapper.py` | Tone Mapper | `scripts/` | ✅ Exists |
| `validate_proposal.py` | Validate Proposal | `scripts/` | ✅ Exists |
| `aggregate_evidence_01.py` | Aggregate Evidence 01 | `scripts/` | ✅ Exists |

### Script Dependencies
- **Input:** JOB-POST.md (raw job posting), portfolio references, market benchmarks
- **Output:** 7 JSON/MD artifacts (jobpost-analysis, tone-map, pricing-analysis, humanization-log, PROPOSAL, proposal-summary, evidence-manifest)
- **External Dependencies:** Python 3.8+, TextBlob for sentiment analysis, JSON Schema validator

### Automation Hooks
- **Pre-execution:** Environment setup, prerequisite validation
- **During execution:** Gate validation scripts at each phase
- **Post-execution:** Evidence aggregation, manifest generation, cleanup scripts
...
**Action:** Execute environment setup activities
...
Evidence: Track artifacts and validation results in `.artifacts/protocol-01/workflow-logs/`
...
**Action:** Execute job post extraction activities
...
**Action:** Execute tone calibration activities
...
**Action:** Execute pricing analysis activities
...
**Action:** Execute proposal drafting activities
...
**Action:** Execute validation activities
...
- **Sequential:** STEP 1 → STEP 2 → STEP 6 (must complete in order)
- **Parallel:** None (all steps sequential)
- **Conditional:** Halt if validation fails, escalate to supervisor

### Workflow State Management

- State stored in: `.artifacts/protocol-01/workflow-state.json`
- Checkpoint validation at each step boundary
- Rollback procedure if step fails: Return to previous step and remediate

### Workflow Monitoring

- Real-time status: `.artifacts/protocol-01/workflow-status.json`
- Execution logs: `.artifacts/protocol-01/workflow-logs/`
- Performance metrics: `.artifacts/protocol-01/workflow-metrics.json`
...
- [ ] All artifacts generated and stored in `.artifacts/protocol-01/`
- [ ] Evidence manifest complete with checksums
- [ ] Quality gates passed (all 5 gates show PASS status)
- [ ] Downstream protocol owner notified and ready
- [ ] No blocking issues or waivers pending

### Handoff Package Contents
- **Evidence Bundle:** `PROTOCOL-01-EVIDENCE.zip` containing:
...
- **Next Protocol Brief:** Clear handoff to Protocol 02
...
```
[PROTOCOL 01 | PHASE X START] - [Action description]
[PROTOCOL 01 | PHASE X COMPLETE] - [Outcome with evidence reference]
[PROTOCOL 01 ERROR] - [Error type and resolution]
```

### Stakeholder Notifications
- **Primary Stakeholder:** Proposal Writer/Freelancer - Notification method: Email/Slack
- **Secondary Stakeholders:** Client, Technical Lead - Notification method: Email
- **Escalation Path:** Technical Lead → Project Manager → Executive if critical issues arise

### Feedback Collection
- Collect feedback from Protocol 02 owner on proposal quality
- Document any concerns or improvement suggestions
- Log feedback in `.artifacts/protocol-01/feedback-log.json`

### Communication Cadence
- Daily status updates during proposal generation
...
- `PROTOCOL_ID=01` - Protocol identifier
- `WORKSPACE_ROOT=.` - Root workspace directory
- `ARTIFACTS_DIR=.artifacts/protocol-01/` - Artifacts storage location
- `LOG_LEVEL=INFO` - Logging verbosity (DEBUG, INFO, WARNING, ERROR)

**Required Permissions:**
- Read access to: `.cursor/ai-driven-workflow/01-*.md`, `.artifacts/`
- Write access to: `.artifacts/protocol-01/`, `scripts/logs/`
...
```
**Flags:**
- `--protocol 01` - Protocol ID to validate
- `--workspace .` - Workspace root directory
- `--strict` - Enforce strict validation

**Output:** `.artifacts/protocol-01/prerequisites-validation.json`
...
python3 scripts/run_protocol_gates.py \
  --protocol 01 \
  --input .artifacts/protocol-01/input/ \
  --output .artifacts/protocol-01/output/ \
  --log-file .artifacts/protocol-01/execution.log \
  --error-handling retry
```
**Flags:**
- `--protocol 01` - Protocol ID
...
python3 scripts/aggregate_evidence_01.py \
  --protocol 01 \
  --artifacts-dir .artifacts/protocol-01/ \
  --output-manifest \
  --checksum sha256
```
**Flags:**
...
**Output:** `.artifacts/protocol-01/EVIDENCE-MANIFEST.json`
**Exit Codes:** 0=success, 1=aggregation failed

#### Command 4: Post-Execution Validation
```bash
python3 scripts/validate_protocol_01.py \
  --protocol 01 \
  --artifacts-dir .artifacts/protocol-01/ \
...
**Output:** `.artifacts/protocol-01/validation-report.json`
...
**Log Files:**
- `.artifacts/protocol-01/execution.log` - Main execution log
- `.artifacts/protocol-01/validation.log` - Validation log
- `.artifacts/protocol-01/error.log` - Error log (if any)

**Status Files:**
- `.artifacts/protocol-01/workflow-status.json` - Real-time status
- `.artifacts/protocol-01/workflow-metrics.json` - Performance metrics
