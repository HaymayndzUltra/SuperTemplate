...
Evidence: Track initialization in `.artifacts/protocol-09/workflow-logs/`
...
Evidence: Store artifacts in `.artifacts/protocol-09/`
...
- State stored in: `.artifacts/protocol-09/workflow-state.json`
- Checkpoint validation at each step boundary
- Rollback procedure if step fails: Return to previous step and remediate

### Workflow Monitoring

- Real-time status: `.artifacts/protocol-09/workflow-status.json`
- Execution logs: `.artifacts/protocol-09/workflow-logs/`
- Performance metrics: `.artifacts/protocol-09/workflow-metrics.json`
...
- `ARTIFACTS_DIR=.artifacts/protocol-09/` - Artifacts storage location
- `LOG_LEVEL=INFO` - Logging verbosity (DEBUG, INFO, WARNING, ERROR)

**Required Permissions:**
- Read access to: `.cursor/ai-driven-workflow/09-*.md`, `.artifacts/`
- Write access to: `.artifacts/protocol-09/`, `scripts/logs/`
...
**Output:** `.artifacts/protocol-09/prerequisites-validation.json`
...
python3 scripts/run_protocol_gates.py \
  --protocol 09 \
  --input .artifacts/protocol-09/input/ \
  --output .artifacts/protocol-09/output/ \
  --log-file .artifacts/protocol-09/execution.log \
...
python3 scripts/aggregate_evidence_09.py \
  --protocol 09 \
  --artifacts-dir .artifacts/protocol-09/ \
  --output-manifest \
  --checksum sha256
```
**Flags:**
...
**Output:** `.artifacts/protocol-09/EVIDENCE-MANIFEST.json`
...
**Output:** `.artifacts/protocol-09/validation-report.json`
**Exit Codes:** 0=all gates pass, 1=gate failure, 2=critical error

### Error Handling & Fallback Procedures

**If Command 1 (Prerequisites) Fails:**
1. Check log: `.artifacts/protocol-09/prerequisites-validation.json`
...
1. Check log: `.artifacts/protocol-09/execution.log`
...
**Log Files:**
- `.artifacts/protocol-09/execution.log` - Main execution log
- `.artifacts/protocol-09/validation.log` - Validation log
- `.artifacts/protocol-09/error.log` - Error log (if any)

**Status Files:**
- `.artifacts/protocol-09/workflow-status.json` - Real-time status
- `.artifacts/protocol-09/workflow-metrics.json` - Performance metrics
...
✅ All artifacts stored in `.artifacts/protocol-09/`
...
- [ ] All artifacts generated and stored in `.artifacts/protocol-09/`
...
- Log feedback in `.artifacts/protocol-09/feedback-log.json`
