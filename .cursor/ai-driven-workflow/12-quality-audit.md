...
----------------|---------|----------|--------|
| `validate_gate_12_*.py` | Gate validation | `scripts/` | ✅ Exists |
| `verify_protocol_12.py` | Protocol verification | `scripts/` | ✅ Exists |
| `generate_artifacts_12.py` | Artifact generation | `scripts/` | ✅ Exists |
| `aggregate_evidence_12.py` | Evidence aggregation | `scripts/` | ✅ Exists |
...
Evidence: Track initialization in `.artifacts/protocol-12/workflow-logs/`
...
Evidence: Store artifacts in `.artifacts/protocol-12/`
...
- State stored in: `.artifacts/protocol-12/workflow-state.json`
- Checkpoint validation at each step boundary
- Rollback procedure if step fails: Return to previous step and remediate

### Workflow Monitoring

- Real-time status: `.artifacts/protocol-12/workflow-status.json`
- Execution logs: `.artifacts/protocol-12/workflow-logs/`
- Performance metrics: `.artifacts/protocol-12/workflow-metrics.json`
...
- `PROTOCOL_ID=12` - Protocol identifier
- `WORKSPACE_ROOT=.` - Root workspace directory
- `ARTIFACTS_DIR=.artifacts/protocol-12/` - Artifacts storage location
- `LOG_LEVEL=INFO` - Logging verbosity (DEBUG, INFO, WARNING, ERROR)

**Required Permissions:**
- Read access to: `.cursor/ai-driven-workflow/12-*.md`, `.artifacts/`
- Write access to: `.artifacts/protocol-12/`, `scripts/logs/`
...
```bash
python3 scripts/validate_prerequisites_12.py \
  --protocol 12 \
  --workspace . \
  --strict
```
**Flags:**
- `--protocol 12` - Protocol ID to validate
- `--workspace .` - Workspace root directory
- `--strict` - Enforce strict validation

**Output:** `.artifacts/protocol-12/prerequisites-validation.json`
...
python3 scripts/run_protocol_gates.py \
  --protocol 12 \
  --input .artifacts/protocol-12/input/ \
  --output .artifacts/protocol-12/output/ \
  --log-file .artifacts/protocol-12/execution.log \
  --error-handling retry
```
**Flags:**
- `--protocol 12` - Protocol ID
...
```bash
python3 scripts/aggregate_evidence_12.py \
  --protocol 12 \
  --artifacts-dir .artifacts/protocol-12/ \
  --output-manifest \
  --checksum sha256
```
**Flags:**
...
**Output:** `.artifacts/protocol-12/EVIDENCE-MANIFEST.json`
**Exit Codes:** 0=success, 1=aggregation failed

#### Command 4: Post-Execution Validation
```bash
python3 scripts/validate_protocol_12.py \
  --protocol 12 \
  --artifacts-dir .artifacts/protocol-12/ \
...
**Output:** `.artifacts/protocol-12/validation-report.json`
**Exit Codes:** 0=all gates pass, 1=gate failure, 2=critical error

### Error Handling & Fallback Procedures

**If Command 1 (Prerequisites) Fails:**
1. Check log: `.artifacts/protocol-12/prerequisites-validation.json`
...
1. Check log: `.artifacts/protocol-12/execution.log`
...
**Log Files:**
- `.artifacts/protocol-12/execution.log` - Main execution log
- `.artifacts/protocol-12/validation.log` - Validation log
- `.artifacts/protocol-12/error.log` - Error log (if any)

**Status Files:**
- `.artifacts/protocol-12/workflow-status.json` - Real-time status
- `.artifacts/protocol-12/workflow-metrics.json` - Performance metrics
...
✅ All artifacts stored in `.artifacts/protocol-12/`
...
- [ ] All artifacts generated and stored in `.artifacts/protocol-12/`
...
- **Evidence Bundle:** `PROTOCOL-12-EVIDENCE.zip` containing:
...
- **Next Protocol Brief:** Clear handoff to Protocol 13
...
```
[PROTOCOL 12 | PHASE X START] - [Action description]
[PROTOCOL 12 | PHASE X COMPLETE] - [Outcome with evidence reference]
[PROTOCOL 12 ERROR] - [Error type and resolution]
```

### Stakeholder Notifications
- **Primary Stakeholder:** QA Manager - Notification method: [Email/Slack/Meeting]
- **Secondary Stakeholders:** Technical Lead, Product Manager, Client - Notification method
...
- Log feedback in `.artifacts/protocol-12/feedback-log.json`
