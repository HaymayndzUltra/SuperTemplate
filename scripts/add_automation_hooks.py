#!/usr/bin/env python3
"""
Add AUTOMATION HOOKS sections to protocols that are missing them
"""
...
def build_automation_hooks_section(protocol_num, registry):
    """Build automation hooks section"""
    
    scripts = get_protocol_scripts(protocol_num, registry)
    
    section = f"""## AUTOMATION HOOKS
...
1. Check log: `.artifacts/protocol-{protocol_num:02d}/prerequisites-validation.json`
...
1. Check log: `.artifacts/protocol-{protocol_num:02d}/execution.log`
...
def add_automation_hooks(file_path, protocol_num, registry):
    """Add AUTOMATION HOOKS section before HANDOFF CHECKLIST"""
    ...
    if "## AUTOMATION HOOKS" in content:
        print(f"Protocol {protocol_num:02d}: AUTOMATION HOOKS already exists")
        return False
    
    # Find HANDOFF CHECKLIST
    if "## HANDOFF CHECKLIST" not in content:
        print(f"Protocol {protocol_num:02d}: No HANDOFF CHECKLIST found")
        return False
    
    # Build section
    section = build_automation_hooks_section(protocol_num, registry)
    
    # Insert before HANDOFF CHECKLIST
    content = content.replace("## HANDOFF CHECKLIST", section + "\n## HANDOFF CHECKLIST")
...
def main():
    ...
    for protocol_num in range(1, 24):
        ...
        if add_automation_hooks(file_path, protocol_num, registry):
            print(f"✅ Protocol {protocol_num:02d} AUTOMATION HOOKS added")
        else:
